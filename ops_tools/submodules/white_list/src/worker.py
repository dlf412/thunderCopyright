...
def get_global_vars(cfg):
    ...
    gv.d = dbpc.dbpc(dbpc_host,int(dbpc_port),dppc_service,"ops_tools_white_list",int(interval))
...
def init_logger(cf):
    ...
    if cf['log'].has_key('logfile'):
        ...
        gv.g_logger.init_logger(
            "ops_tools#white_list", log_level, log_file, SysLogHandler.LOG_LOCAL2)
    else:
        gv.g_logger.init_logger(
            "ops_tools#white_list", log_level, 'syslog', SysLogHandler.LOG_LOCAL2)
    ...
    while True:
        with Connection(gv.tmphigh_url) as conn:
            try:
                ...
                gv.g_logger.info('ops_tools white list start')
                worker.run()
            except Exception:
                gv.g_logger.error("ops_tools white list start %s happend!" % str(traceback.format_exc()))
    gv.d.join()
